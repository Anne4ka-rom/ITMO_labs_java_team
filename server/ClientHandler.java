...
public class ClientHandler { // объявляет класс для обработки клиентских подключений
    ...
    private Object attachment; // объект для хранения вложения (менеджер коллекции для пользователя)
    
    /**
     * Создаёт новый обработчик для подключенного клиента
     * 
    ...
    public ClientHandler(SocketChannel channel, Selector selector) { // конструктор класса
        ...
        this.pendingData = new ByteArrayOutputStream(); // создаём новый поток для накопления данных
        ...
        this.attachment = null; // инициализируем вложение как null
    }
    ...
    public Selector getSelector() { // геттер для поля selector
        return selector; // возвращаем сохранённый мультиплексор
    }
    ...
    public SocketChannel getChannel() { // геттер для поля channel
        return channel; // возвращаем сохранённый канал
    }
    ...
    /**
     * Очищает сохранённый полный запрос
    ...
    /**
     * Устанавливает объект вложения (менеджер коллекции) для этого клиента
     * 
     * @param attachment объект для хранения (обычно CollectionManager)
     */
    public void setAttachment(Object attachment) { // сеттер для поля attachment
        this.attachment = attachment; // сохраняем переданный объект вложения
    }
    
    /**
     * Возвращает объект вложения (менеджер коллекции) для этого клиента
     * 
     * @return объект вложения или null если не установлен
     */
    public Object getAttachment() { // геттер для поля attachment
        return attachment; // возвращаем сохранённый объект вложения
    }
    ...
    public void close() { // метод закрытия и очистки ресурсов
        ...
        attachment = null; // удаляем ссылку на вложение
    }
}