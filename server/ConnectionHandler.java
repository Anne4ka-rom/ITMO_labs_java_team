...
import common.CommandType; // для использования перечисления типов команд (нужен для проверки EXIT)
...
import java.io.EOFException; // для обработки ситуации, когда клиент неожиданно закрыл соединение
...
public class ConnectionHandler implements Runnable { // объявление класса ConnectionHandler, реализующего интерфейс Runnable
    ...
    public void run() { // метод run() из интерфейса Runnable
        ...
        try { // начало блока перехвата исключений (основная логика обработки клиента)
            ...
            while (true) { // бесконечный цикл для приёма команд от клиента (выход только при ошибке или команде EXIT)
                try { // внутренний try для обработки ошибок чтения отдельной команды
                    Request request = reader.readRequest(); // чтение и десериализация запроса от клиента (блокирующая операция)
                    ...
                    if (request.getCommand().getType() == CommandType.EXIT) { // проверка, является ли команда командой завершения
                        System.out.println("Клиент завершил сессию"); // вывод сообщения о завершении сессии клиентом
                        break; // выход из бесконечного цикла while
                    }
                } catch (EOFException e) { // обработка ситуации, когда клиент неожиданно закрыл соединение (конец файла)
                    System.out.println("Клиент отключился"); // вывод сообщения об отключении клиента
                    break; // выход из бесконечного цикла while
                }
            } // конец внутреннего try-catch и цикла while
        ...
        } finally { // блок, который выполнится всегда (и при успехе, и при ошибке)
            // закрытие ресурсов
...
} // конец класса ConnectionHandler