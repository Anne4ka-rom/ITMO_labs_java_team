 ...
 * грузоподъемность, тип транспортного средства и тип топлива и владельца.
...
public class Vehicle implements Comparable<Vehicle>, Serializable { // объявляем класс Vehicle, реализующий интерфейс Comparable
    ...
    private String ownerUsername; // имя пользователя, создавшего этот объект
     ...
     * Владелец должен быть установлен отдельно через {@link #setOwnerUsername(String)}
     * 
    ...
    /**
     * Возвращает имя пользователя, создавшего это транспортное средство
     * 
     * @return имя владельца транспортного средства
     */
    public String getOwnerUsername() { // геттер для поля ownerUsername
        return ownerUsername; // возвращает имя владельца транспортного средства
    }

    /**
     * Устанавливает имя пользователя, создавшего это транспортное средство
     * 
     * @param ownerUsername имя владельца транспортного средства (не может быть null или пустым)
     * @throws IllegalArgumentException если ownerUsername == null или ownerUsername.trim().isEmpty()
     */
    public void setOwnerUsername(String ownerUsername) { // сеттер для поля ownerUsername
        if (ownerUsername == null || ownerUsername.trim().isEmpty()) { // проверяем, что строка не null и не пустая после обрезки пробелов
            throw new IllegalArgumentException("Owner username не может быть null или пустым"); // выбрасываем исключение с сообщением об ошибке
        }
        this.ownerUsername = ownerUsername; // присваиваем значение после проверки
    }
     ...
     * enginePower=150.50, capacity=2000.00, type=CAR, fuelType=GASOLINE, owner='username'}
     * 
    ...
    public String toString() {
        return String.format("Vehicle{id=%d, name='%s', coordinates=%s, date=%s, " +
                "enginePower=%.2f, capacity=%.2f, type=%s, fuelType=%s, owner='%s'}",
                id, name, coordinates, creationDate, enginePower, capacity, type, fuelType, ownerUsername); // форматированный вывод всех полей объекта, где %d -- целое число, %s -- строка, %.2f -- число с двумя знаками после запятой
    }
